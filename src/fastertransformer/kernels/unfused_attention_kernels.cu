...
__global__ void glm_alpha_add_fusedQKV_bias_transpose_kernel(T* q_buf,
                                                            ...
                                                            const T* __restrict QKV,
                                                            const T* __restrict qkv_bias,
                                                            const int* __restrict input_lengths,
                                                            const int* __restrict mask_positions,
                                                            const int layer_id,
                                                            const int batch_size,
                                                            const int seq_len,
                                                            const int head_num,
                                        ...
                                        cudaStream_t stream)
{
    ...
    glm_alpha_add_fusedQKV_bias_transpose_kernel<<<grid, block, 0, stream>>>(
            q_buf, k_buf, v_buf, QKV, qkv_bias, input_lengths, mask_positions, layer_id, batch_size, seq_len, head_num, size_per_head, -rotary_embedding_dim);
}
                                        ...
                                        cudaStream_t stream)
{
    ...
    dim3 grid(seq_len, head_num, batch_size);
    glm_alpha_add_fusedQKV_bias_transpose_kernel<<<grid, block, 0, stream>>>(
            q_buf, k_buf, v_buf, QKV, qkv_bias, input_lengths, mask_positions, layer_id, batch_size, seq_len, step, head_num, size_per_head, -rotary_embedding_dim);
}
