...
__global__ void int4WeightPerChannelLdkMultiplication(
    const char2* weight, const half4* input, const float* scale_list, void* output, const int k_4)
{
    const int tidx = threadIdx.x;
    const int bidx = blockIdx.x;
    const int row_idx = bidx * nPerThread;
...
#define RUN4(M, TYPE)                                                                                                  \
    int4WeightPerChannelLdkMultiplication<M, nPerThread><<<grid, block, shm_size, stream>>>(                           \
        (const char2*)weight, (const TYPE*)input, scale_list, (void*)output, k / 4);
...
template<typename T>
__global__ void
int4WeightExtractionDevice(const int8_t* weight, const float* scale_list, T* output, const int n, const int k)
{
    for (int i = blockIdx.x * k + threadIdx.x; i < blockIdx.x * k + k; i += blockDim.x) {
        ...
        low = low >> 4;
        output[i * 2] = T(high * 1.0f) * T(scale_list[blockIdx.x]);
        output[i * 2 + 1] = T(low * 1.0f) * T(scale_list[blockIdx.x]);
    }
}

template<typename T>
void invokeInt4WeightExtraction(
    const int8_t* weight, const float* scale_list, T* output, const int n, const int k, cudaStream_t stream)
{
    dim3 grid(n);
    dim3 block(256);
...
template void invokeInt4WeightExtraction(
    const int8_t* weight, const float* scale_list, half* output, const int n, const int k, cudaStream_t stream);

#ifdef ENABLE_BF16
template void invokeInt4WeightExtraction(const int8_t* weight,
                                         const float* scale_list,
                                         __nv_bfloat16* output,
                                         const int n,
                                         const int k,
