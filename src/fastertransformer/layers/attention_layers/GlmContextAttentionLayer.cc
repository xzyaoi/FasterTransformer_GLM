...
#include "src/fastertransformer/layers/attention_layers/GlmContextAttentionLayer.h"
#include "src/fastertransformer/kernels/unfused_attention_kernels.h"

namespace fastertransformer {

template<typename T>
void GlmContextAttentionLayer<T>::forward(std::vector<fastertransformer::Tensor>* output_tensors,
                                          ...
                                          const AttentionWeight<T>* attention_weights)
{
    ...
    if(attention_weights->query_weight.kernel) {
        ...
        invokeInt4WeightExtraction(attention_weights->query_weight.int8_kernel,
...
#endif
    sync_check_cuda_error();
    invokeGlmAddFusedQKVBiasTranspose(q_buf_2_,
                                   k_buf_2_,
                                   v_buf_2_,
                                   qkv_buf_,
    ...
    if (is_final == false) {
        ...
        if(attention_weights->attention_output_weight.kernel) {
            ...
            invokeInt4WeightExtraction(attention_weights->attention_output_weight.int8_kernel,
...
template<typename T>
GlmContextAttentionLayer<T>::GlmContextAttentionLayer(size_t max_batch_size,
                                                      size_t max_seq_len,
                                                      size_t head_num,
...
template<typename T>
GlmContextAttentionLayer<T>::GlmContextAttentionLayer(GlmContextAttentionLayer<T> const& attention_layer):
...
template<typename T>
GlmContextAttentionLayer<T>::~GlmContextAttentionLayer()
{
...
template<typename T>
void GlmContextAttentionLayer<T>::allocateBuffer()
{
    FT_CHECK(false);
...
template<typename T>
void GlmContextAttentionLayer<T>::allocateBuffer(size_t batch_size, size_t seq_len)
{
...
template<typename T>
void GlmContextAttentionLayer<T>::freeBuffer()
{
...
template<typename T>
bool GlmContextAttentionLayer<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
bool GlmContextAttentionLayer<T>::isValidSeqLen(size_t seq_len)
{
...
template class GlmContextAttentionLayer<float>;
template class GlmContextAttentionLayer<half>;
#ifdef ENABLE_BF16
template class GlmContextAttentionLayer<__nv_bfloat16>;
#endif
