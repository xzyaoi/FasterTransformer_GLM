...
#include "src/fastertransformer/layers/attention_layers/GlmDecoderSelfAttentionLayer.h"
...
template<typename T>
void fusedQKV_masked_attention_dispatch(const T* q_buf,
                                        const T* k_buf,
                                        const T* v_buf,
                                        ...
                                        cudaStream_t stream)
{
    ...
    params.inv_sqrt_dh = layer_id + 1.0f;
...
template void fusedQKV_masked_attention_dispatch(const half* q_buf,
                                                 const half* k_buf,
                                                 const half* v_buf,
...
template<typename T>
void GlmDecoderSelfAttentionLayer<T>::allocateBuffer()
{
...
template<typename T>
void GlmDecoderSelfAttentionLayer<T>::allocateBuffer(size_t batch_size)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    qkv_buf_ =
        reinterpret_cast<T*>(allocator_->reMalloc(qkv_buf_, sizeof(T) * batch_size * 3 * local_hidden_units_, false));
    q_buf_ =
        reinterpret_cast<T*>(allocator_->reMalloc(q_buf_, sizeof(T) * batch_size * local_hidden_units_, false));
    k_buf_ =
        reinterpret_cast<T*>(allocator_->reMalloc(k_buf_, sizeof(T) * batch_size * local_hidden_units_, false));
    v_buf_ =
...
template<typename T>
void GlmDecoderSelfAttentionLayer<T>::freeBuffer()
{
...
template<typename T>
bool GlmDecoderSelfAttentionLayer<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
GlmDecoderSelfAttentionLayer<T>::GlmDecoderSelfAttentionLayer(size_t max_batch_size,
                                                        size_t head_num,
...
GlmDecoderSelfAttentionLayer<T>::GlmDecoderSelfAttentionLayer(size_t max_batch_size,
                                                        ...
                                                        bool sparse,
                                                        int int8_mode):
    GlmDecoderSelfAttentionLayer<T>(max_batch_size,
                                 head_num,
                                 size_per_head,
                                 head_num,
                                 0,
...
template<typename T>
GlmDecoderSelfAttentionLayer<T>::GlmDecoderSelfAttentionLayer(GlmDecoderSelfAttentionLayer<T> const& attention_layer):
    GlmDecoderSelfAttentionLayer<T>(attention_layer.max_batch_size_,
...
template<typename T>
GlmDecoderSelfAttentionLayer<T>::~GlmDecoderSelfAttentionLayer()
{
...
template<typename T>
void GlmDecoderSelfAttentionLayer<T>::forward(std::vector<fastertransformer::Tensor>* output_tensors,
                                           ...
                                           const AttentionWeight<T>* attention_weights)
{
    ...
    invokeGlmAddFusedQKVBiasTranspose(q_buf_,
                                        k_buf_,
                                        v_buf_,
                                        qkv_buf_,
    ...
    fusedQKV_masked_attention_dispatch<T>(q_buf_,
                                          k_buf_,
                                          v_buf_,
    ...
    if (sparse_ && cublas_wrapper_->isUseSparse(1, d_model_, m_padded, local_hidden_units_)) {
        ...
        if (int8_mode_ != 0 && batch_size <= 2) {
            ...
            if(attention_weights->attention_output_weight.kernel) {
                ...
                int4WeightPerChannelLdkMultiplicationLauncher(attention_weights->attention_output_weight.int8_kernel,
                                                          context_buf_,
                                                          attention_weights->attention_output_weight.scale,
                                                          attention_out,
                                                          batch_size,
...
template class GlmDecoderSelfAttentionLayer<float>;
template class GlmDecoderSelfAttentionLayer<half>;
#ifdef ENABLE_BF16
template class GlmDecoderSelfAttentionLayer<__nv_bfloat16>;
#endif
