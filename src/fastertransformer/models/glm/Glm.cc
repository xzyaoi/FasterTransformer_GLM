...
template<typename T>
Glm<T>::Glm(Glm<T> const& glm):
    BaseLayer(glm),
    head_num_(glm.head_num_),
    size_per_head_(glm.size_per_head_),
    inter_size_(glm.inter_size_),
    num_layer_(glm.num_layer_),
    vocab_size_(glm.vocab_size_),
    rotary_embedding_dim_(glm.rotary_embedding_dim_),
    start_id_(glm.start_id_),
    end_id_(glm.end_id_),
    hidden_units_(glm.hidden_units_),
    tensor_para_(glm.tensor_para_),
    pipeline_para_(glm.pipeline_para_),
    local_head_num_(glm.local_head_num_),
    vocab_size_padded_(glm.vocab_size_padded_),
    custom_all_reduce_comm_(glm.custom_all_reduce_comm_),
    enable_custom_all_reduce_(glm.enable_custom_all_reduce_)
{
    initialize();
}
                    ...
                    const GlmWeight<T>* glm_weights)
{
    ...
    has_diff_runtime_args = hasDiffRuntimeArgs(input_tensors);

    has_per_item_requested_length = input_tensors->at("max_output_seq_len").shape.size() > 1;

    sequence_lengths = (int*)(output_tensors->at("sequence_length").data);
    data_type = getTensorType<T>();

    output_ids_buf_ = (int*)(output_tensors->at("output_ids_buf").data);
    logits_buf_ = (float*)(output_tensors->at("logits_buf").data);
    ...
    if (vocab_size_ == vocab_size_padded_) {
        padded_embedding_kernel_ptr_ = glm_weights->post_decoder_embedding.kernel;
    }
    else {
        printf("vocab_size_ != vocab_size_padded_, check where you init logits_buf.");
        cudaMemcpyAsync(padded_embedding_kernel_,
                        glm_weights->post_decoder_embedding.kernel,
